...
After modifying the driver code to support PDM as mentioned in the 'Implementation' section of this chapter,
the program no longer outputs any data (only 0 byte values), and the output of the oscilloscope is shown in Figure~\ref{fig:pdm_no_config}.
It becomes immediately clear that there is still something missing.
The reason for this behavior is that the PDM clock of the board had been disabled after causing problems, due to PDM not being enabled.
...
Now that the PDM microphone is enabled and working, it is possible to conduct the aforementioned test and
evaluate the data.
